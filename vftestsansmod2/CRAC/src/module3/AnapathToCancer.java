...
public class AnapathToCancer extends DAO<Cancer>{
	...
	public ArrayList<Patient> Cim10toCancer( ArrayList<Patient> arr_pa ) {
	...
	public ArrayList<Patient> AdicaptoCancer( ArrayList<Patient> arr_pa ) {
        // connecter la base de données
        ...
        for (Patient pa  : arr_pa) {
            	...
            	for (IdentiteAdministrative ida : pa.getIDa()){
            		...
            		for (Diagnostic dia : ida.getDiag()){
            			...
            			if (dia.getTypediag() == "adicap" && dia.getCodediag().length()>7) {
            				
            				String lesion_patient = dia.getCodediag();
            				if (lesion_patient.length() > 8) {
            				lesion_patient = dia.getCodediag().substring(4,8);}
            				else {lesion_patient = dia.getCodediag().substring(4);}
